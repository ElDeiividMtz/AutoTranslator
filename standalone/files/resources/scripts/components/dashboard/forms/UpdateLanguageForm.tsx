...
// Override from server config (admin can customize)
function getFlagCountry(langCode: string): string {
    ...
    if (serverFlags?.[langCode]) {
        // Server flags are emoji or country codes — extract 2-letter code
        const flag = serverFlags[langCode];
        // If it's a 2-letter code already, use it
...
function flagUrl(langCode: string): string {
    const country = getFlagCountry(langCode);
